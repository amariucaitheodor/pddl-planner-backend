class Predicate(object):
    """
        ...
        Methods:
            ...
            ground:    Takes in an iterable of tuples that contain a pair of strings. The first is a variable name
            and the second is an object name. It should throw an error if the variable type and the object type do
            not match. It should also throw an error if the set of variables passed in isn't a subset of the
            variables in args. The effect should be to remove the appropriate elements from the args list,
    ...
    def export(self, lvl=1, sp="  ", untyped=False, grounding=None):
        ...
        if self.ground_args is None:
            ...
            arguments = self.ground_args

        if len(arguments) > 0:
            sep = " "

        if not untyped and len(arguments) > 0 and arguments[0][1] != Predicate.OBJECT:
            arg_s = " ".join(["%s - %s" % (v, t) for v, t in arguments])
        else:
            arg_s = " ".join([v for v, t in arguments])
    ...
    def ground(self, it):
        """Takes in an iterable of tuples that contain a pair of strings. The first is a variable name and the second
        is an object name. It should throw an error if the variable type and the object type do not match. It should
        also throw an error if the set of variables passed in isn't a subset of the variables in args. The effect
        should be to remove the appropriate elements from the args list, and add the appropriate elements to the
        ground_args list.
        """

        assert hasattr(it, '__iter__') and \
               all([isinstance(item, tuple) and
                    len(item) == 2 and
                    isinstance(item[0], str) and
                    isinstance(item[1], str) for item in it]), \
    ...
    def __str__(self):
        ...
        if self.ground_args is None:
            return "%s (%s)" % (self.name, ", ".join(["%s %s" % (arg[1], arg[0]) for arg in self.args]))
        else:
