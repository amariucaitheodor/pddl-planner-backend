...
class GroundProblem(Problem):
    ...
    Houses the basic data structures for a contingent processing problem that has
    ...
    def is_equal(self, p):
        ...
        if not all([sa == pa for sa, pa in zip(sorted(list(self.operators)),
                                               sorted(list(p.operators)))]):
            print("operators")
            return False

        if not all([sp == pp for sp, pp in zip(sorted(list(self.fluents)), sorted(list(p.fluents)))]):
            print("fluents")
            print("*self*")
    ...
    def _get_unassigned_vars(self, formula, assigned):
        ...
        # if isinstance(formula, Forall): try: for v, t in formula.params: assigned[(v, hash(formula))] =
        # self.type_to_obj[t] except KeyError as e: raise KeyError("Cannot get unassigned vars list due to bad
        # parsing of forall object: %s" % str(formula)) elif isinstance(formula, Primitive):
    ...
    def _create_valuations(self, params):
        """
        Input: params            list of tuples, where the first item is the parameter name, and the second is the
        parameter type
    ...
    def _predicate_to_fluent(self, predicate, assignment, fluent_dict=None):
        """
        Inputs:
            predicate            The predicate to be converted
            assignment            a dictionary mapping each possible variable name to an object

        Returns: A fluent that has the particular valuation for the variables as given in input. The old predicate is
        *untouched*
        """
    ...
    def _partial_ground_formula(self, formula, assignment, fluent_dict):
        ...
        Returns: A formula that has the particular valuation for the variables as given in input. The old formula is
        *untouched*
        """
    ...
    def _create_operators(self, fluent_dict):
        ...
        for a in self.actions:
            ...
            var_names, val_generator = self._create_valuations(a.parameters)
    ...
    def _get_unground_vars(self, formula, d):
        ...
        if isinstance(formula, Forall):
            d[(formula.params, hash(formula))] = d.t
