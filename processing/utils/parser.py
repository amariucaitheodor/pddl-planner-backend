...
class Problem(object):
    ...
    def _parse_domain(self, f_domain):
        ...
        self.predicates = [self.to_predicate(c, given_map=const_map) for c in parse_tree[":predicates"].children]
    ...
    def to_action(self, node):
        ...
        if ":observe" in node:
            ...
            observe = self.to_predicate(node[":observe"].children[0], given_map=parameter_map)
        else:
            observe = None
    ...
    def to_predicate(self, node, f='predicate', given_map=None):
        """
        ...
        # here is where the givenMap comes in...
        if given_map is None:
            if 'predicate' == f:
    ...
    def to_formula(self, node, parameter_map=None):
        ...
        if "forall" == node.name:
            ...
            if len(node.children) == 2 and len(node.children[0].children) > 0:
                ...
                read_child = PDDL_Utils.read_type(new_child)

            for v, t in read_child:
                parameter_map[v] = t
            args = [self.to_formula(c, parameter_map) for c in node.children[i:]]
            for v, t in read_child:
                del (parameter_map[v])
            return Forall(read_child, args)
        ...
        if "and" == node.name:
            ...
            # make p != p2, otherwise might run into issues with mutation in some later step
            p = Primitive(self.to_predicate(node.children[0], given_map=parameter_map))
            p2 = Primitive(self.to_predicate(node.children[0], given_map=parameter_map))
            return Xor([p, Not([p2])])
        elif "when" == node.name:
            assert len(args) == 2, \
            ...
            # it's a predicate
            return Primitive(self.to_predicate(node, given_map=parameter_map))
