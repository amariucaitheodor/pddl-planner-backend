...
class PDDL_Tree(object):
    ...
    def _alter_tree(root):
        ...
        alter_set = {":precondition", ":effect", ":observe"}
        i = 0
    ...
    def _get_pddl_list(contents):
        ...
        while i < len(contents_list) - 1:
            if contents_list[i] == "[" and contents_list[i + 1] == " ":
                contents_list.pop(i + 1)
            elif contents_list[i] == " " and contents_list[i + 1] == "]":
                contents_list.pop(i)
                i -= 1
            elif (contents_list[i] == "]" and contents_list[i + 1] == "[") or (contents_list[i] not in ["[", "]", " "]
                                                                               and contents_list[i + 1] == "["):
